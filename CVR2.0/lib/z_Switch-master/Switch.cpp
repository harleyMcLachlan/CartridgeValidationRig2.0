...
// level(0)
Switch::Switch(task_enum taskENUM, byte _pin, byte PinMode, bool polarity, int debounceDelay, int longPressDelay, int doubleClickDelay):
...
{ pinMode(pin, PinMode);
  ...
  task = taskENUM;
  ticking_state = SW_TICKING_ACTIVE;
}

void Switch::stop()
{
  this->level = 0;
  this->ticking_state = SW_TICKING_NOT_ACTIVE;
}

void Switch::start()
{
  this->ticking_state = SW_TICKING_ACTIVE;
}
